...
The format is a custom assembly language, SkyASM, compiled down by the skysa assembler into byte-level semantic instructions, which are then wrapped in 6502 asm to stream each byte over the port. SkyASM is contained within .ssa files (SkyASM Source Assembly). The SPIC then interprets the byte-level instructions and responds accordingly.
