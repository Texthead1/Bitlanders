...
As a `.bat` batch file is used for building, it can only be used to build this system on Windows machines. The ROM should be buildable on other platforms with alternate build processes.

> [!WARNING]
> Due to recent mapper alterations, the current build of the game is extremely unstable. It is not suggested to run the current build.
